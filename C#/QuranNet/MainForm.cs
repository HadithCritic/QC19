...
namespace QuranNet
{
    ...
    {
        // https://alt-codes.net/arrow_alt_codes.php
        ...
        private string m_numerical_system_name = NumericalSystem.DEFAULT_NAME;
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (m_client != null)
                {
                    ...
                    {
                        m_text_mode = m_client.NumericalSystem.TextMode;
        ...
        private void MainForm_KeyDown(object sender, KeyEventArgs e)
        {
            if (ModifierKeys == Keys.Control)
            {
                ...
                {
                    this.Text = Application.ProductName + " 1433" + " - " + Globals.LONG_VERSION;
                }
                else
                {
                    this.Text = Application.ProductName + " 1433" + " - " + Globals.SHORT_VERSION;
                }
            }
            this.Refresh();
        }
            ...
            try
            {
                if (m_client != null)
                {
                    if (TextModeComboBox.SelectedItem != null)
                    {
                        ...
                        m_client.BuildSimplifiedBook(m_text_mode, false, true, false, false, true, false, false, false, false);
                        ...
                        if (NumericalSystemComboBox.Items.Count > 0)
                        {
                            if (m_client.NumericalSystem != null)
                            {
                                ...
                                string letter_valuation = m_client.NumericalSystem.Name.Substring(pos + 1);
                                if (NumericalSystemComboBox.Items.Contains(letter_valuation))
                                {
                                    NumericalSystemComboBox.SelectedItem = letter_valuation;
                                }
                                else
                                {
        ...
        private void NumericalSystemComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (m_client != null)
            {
                if (TextModeComboBox.SelectedItem != null)
                {
                    ...
                    {
                        string valuation_system = NumericalSystemComboBox.SelectedItem.ToString();
                        m_numerical_system_name = m_text_mode + "_" + valuation_system;
        ...
        private void LoadSpecialChapterNumbers()
        {
            ...
            string special_chapters_path = "3dQuran" + Path.DirectorySeparatorChar + "chapters.txt";
            if (File.Exists(special_chapters_path))
            {
                List<string> lines = FileHelper.LoadLines(special_chapters_path);
        ...
        private string m_index_path = "index.html";
        ...
        private void IncludeWordsCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            m_include_similar_words = IncludeWordsCheckBox.Checked;
        }
        ...
        private void SaveLinks()
        {
            if (m_links != null)
            {
                ...
                if (Directory.Exists(DATA_FOLDER))
                {
                    ...
                    if (m_selected_chapter_index == -1)
                    {
                        m_save_path = folder_name + "/" + m_numerical_system_name + "_" + m_link_by.ToString() + "_" + m_link_to.ToString() + ".tsv";
                    }
                    else
                    {
                        m_save_path = folder_name + "/" + "links" + ".tsv";
                    }

                    if (!String.IsNullOrEmpty(m_save_path))
                    {
                        FileHelper.SaveText(m_save_path, str.ToString());
                    }
                    else
                    {
                        MessageBox.Show("Cannot save:\r\n" + m_save_path, Application.ProductName);
                    }
                }
            }
        }
        ...
        {
            if (File.Exists(DATA_FOLDER + "/" + m_index_path))
            {
                if (Directory.Exists(folder_name))
                {
                    File.Copy(DATA_FOLDER + "/" + m_index_path, folder_name + "/" + m_index_path, true);
                }
            }
        }
            ...
            if (str.Length > 0)
            {
                if (Directory.Exists(DATA_FOLDER))
                {
                    if (!String.IsNullOrEmpty(m_save_path))
                    {
                        ...
                        if (m_save_path.Contains("links"))
                        {
                            json_path = m_save_path.Replace("links.tsv", "network.json");
                        }
                        else
                        {
                            json_path = m_save_path.Replace(".tsv", ".json");
                        }
                        ...
                        {
                            MessageBox.Show("Cannot save:\r\n" + json_path, Application.ProductName);
                        }
                    }
                }
            }
