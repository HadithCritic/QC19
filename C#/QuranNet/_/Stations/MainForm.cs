...
namespace QuranNet
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Shown(object sender, EventArgs e)
        {
            ...
            StationsComboBox.Items.Add("All");  // all stations
            for (int i = 1; i <= STATIONS; i++)
            {
                StationsComboBox.Items.Add(i.ToString());
            }
            StationsComboBox.Items.Add("Few");      // speical stations
            StationsComboBox.Items.Add("1by1");     // station by station each in its own filepath
            StationsComboBox.SelectedIndex = STATIONS + 1;
        ...
        private List<int> m_special_station_numbers = null;
        private void LoadSpecialStationNumbers()
        {
            m_special_station_numbers = new List<int>();
            string special_stations_filename = "3dQuran" + Path.DirectorySeparatorChar + "stations.txt";
            if (File.Exists(special_stations_filename))
            {
                List<string> lines = FileHelper.LoadLines(special_stations_filename);
                foreach (string line in lines)
                {
                    int special_station_number;
                    if (int.TryParse(line, out special_station_number))
                    {
                        m_special_station_numbers.Add(special_station_number);
                    }
                }
            }
        }
        private List<int> m_selected_station_numbers = null;
        ...
        private bool m_build_network_stations_1by1 = false;
        private int m_selected_station_index = -1;
        private void StationsComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            m_selected_station_index = StationsComboBox.SelectedIndex;
            if (m_selected_station_index > -1)
            {
                if (m_selected_station_index == StationsComboBox.Items.Count - 1) // station by station each in its own filepath
                {
                    m_build_network_stations_1by1 = true;
                    m_selected_station_numbers = new List<int>();
                    ToolTip.SetToolTip(this.StationsComboBox, "Build network station by station each in its own filepath");
                }
                else if (m_selected_station_index == STATIONS + 1) // few stations
                {
                    m_build_network_stations_1by1 = false;
                    m_selected_station_numbers = m_special_station_numbers;
                    ToolTip.SetToolTip(this.StationsComboBox, "Build network for special stations");
                }
                else if (m_selected_station_index == 0)                                 // all stations
                {
                    m_build_network_stations_1by1 = false;
                    m_selected_station_numbers = new List<int>();
                    for (int i = 1; i <= STATIONS; i++)
                    {
                        m_selected_station_numbers.Add(i);
                    }
                    ToolTip.SetToolTip(this.StationsComboBox, "Build network for all stations");
                }
                else if ((m_selected_station_index > 0) && (m_selected_station_index <= STATIONS))   // a single station
                {
                    m_build_network_stations_1by1 = false;
                    m_selected_station_numbers = new List<int>() { m_selected_station_index };
                    ToolTip.SetToolTip(this.StationsComboBox, "Build network for station " + m_selected_station_index.ToString());
                }
            }
        }
        ...
        private void BuildNodes()
        {
            try
            {
                if (m_selected_station_numbers != null)
                {
                    ...
                    if (m_nodes != null)
                    {
                        ...
                        if (m_client != null)
                        {
                            ...
                            {
                                foreach (Station station in m_client.Book.Stations)
                                {
                                    if (m_selected_station_numbers.Contains(station.Number))
                                    {
                                        foreach (Verse verse in station.Verses)
                                        {
                                            foreach (Word word in verse.Words)
                                            {
                                                ...
                                                node.Id = word.Number;
                                                node.X = station.Number;
                                                node.Y = verse.NumberInStation;
        ...
        private void SaveLinks()
        {
            if (m_links != null)
            {
                ...
                if (Directory.Exists(DATA_FOLDER))
                {
                    ...
                    if (m_selected_station_index == 0)
                    {
                        folder_name += "/" + "all";                             // sub filepath: all stations
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if ((m_selected_station_index >= 1) && (m_selected_station_index <= STATIONS))
                    {
                        folder_name += "/" + m_selected_station_index.ToString();     // sub filepath: a single station
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if (m_selected_station_index == STATIONS + 1)  // few stations
                    ...
                    else if (m_selected_station_index == STATIONS + 2)  // station by station each in its own filepath
                    {
                    }
        ...
        private void BuildNetworkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (m_build_network_stations_1by1)
                {
                    BuildNetworkForEachStations();
                }
                else
                {
                    BuildNetwork();
        ...
        private void BuildNetworkForEachStations()
        {
            for (int i = 1; i <= STATIONS; i++)
            {
                m_selected_station_index = i;
                m_selected_station_numbers = new List<int>() { m_selected_station_index };
        ...
        private void SaveNetwork()
        {
            ...
            if (m_nodes != null)
            {
                ...
                if (m_nodes.Count > 0)
                {
                    // (m_selected_station_index == 0) // all stations
                    foreach (Node node in m_nodes)
                    {
                        str.Append("\t\t{");
                        str.Append("\"id\":" + " " + node.Id.ToString() + ", ");
                        str.Append("\"fx\":" + " " + ((m_build_network_stations_1by1 ? 0 : node.X) * 10 - 800).ToString() + ", ");
