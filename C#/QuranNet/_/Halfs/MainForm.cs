...
namespace QuranNet
{
    public partial class MainForm : Form
    {
        ...
        {
            HalfsComboBox.Items.Clear();
            HalfsComboBox.Items.Add("All");  // all halfs
            for (int i = 1; i <= HALFS; i++)
            {
                HalfsComboBox.Items.Add(i.ToString());
            }
            HalfsComboBox.Items.Add("Few");      // speical halfs
            HalfsComboBox.Items.Add("1by1");     // half by half each in its own filepath
            HalfsComboBox.SelectedIndex = HALFS + 1;
        ...
        private List<int> m_special_half_numbers = null;
        private void LoadSpecialHalfNumbers()
        {
            m_special_half_numbers = new List<int>();
            string special_halfs_filename = "3dQuran" + Path.DirectorySeparatorChar + "halfs.txt";
            if (File.Exists(special_halfs_filename))
            {
                List<string> lines = FileHelper.LoadLines(special_halfs_filename);
                foreach (string line in lines)
                {
                    int special_half_number;
                    if (int.TryParse(line, out special_half_number))
                    {
                        m_special_half_numbers.Add(special_half_number);
                    }
                }
            }
        }
        private List<int> m_selected_half_numbers = null;
        ...
        private bool m_build_network_halfs_1by1 = false;
        private int m_selected_half_index = -1;
        private void HalfsComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            m_selected_half_index = HalfsComboBox.SelectedIndex;
            if (m_selected_half_index > -1)
            {
                if (m_selected_half_index == HalfsComboBox.Items.Count - 1) // half by half each in its own filepath
                {
                    m_build_network_halfs_1by1 = true;
                    m_selected_half_numbers = new List<int>();
                    ToolTip.SetToolTip(this.HalfsComboBox, "Build network half by half each in its own filepath");
                }
                else if (m_selected_half_index == HALFS + 1) // few halfs
                {
                    m_build_network_halfs_1by1 = false;
                    m_selected_half_numbers = m_special_half_numbers;
                    ToolTip.SetToolTip(this.HalfsComboBox, "Build network for special halfs");
                }
                else if (m_selected_half_index == 0)                                 // all halfs
                {
                    m_build_network_halfs_1by1 = false;
                    m_selected_half_numbers = new List<int>();
                    for (int i = 1; i <= HALFS; i++)
                    {
                        m_selected_half_numbers.Add(i);
                    }
                    ToolTip.SetToolTip(this.HalfsComboBox, "Build network for all halfs");
                }
                else if ((m_selected_half_index > 0) && (m_selected_half_index <= HALFS))   // a single half
                {
                    m_build_network_halfs_1by1 = false;
                    m_selected_half_numbers = new List<int>() { m_selected_half_index };
                    ToolTip.SetToolTip(this.HalfsComboBox, "Build network for half " + m_selected_half_index.ToString());
                }
            }
        }
        ...
        private void BuildNodes()
        {
            try
            {
                if (m_selected_half_numbers != null)
                {
                    ...
                    if (m_nodes != null)
                    {
                        ...
                        if (m_client != null)
                        {
                            ...
                            {
                                foreach (Half half in m_client.Book.Halfs)
                                {
                                    if (m_selected_half_numbers.Contains(half.Number))
                                    {
        ...
        private void SaveLinks()
        {
            if (m_links != null)
            {
                ...
                if (Directory.Exists(DATA_FOLDER))
                {
                    ...
                    if (m_selected_half_index == 0)
                    {
                        folder_name += "/" + "all";                             // sub filepath: all halfs
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if ((m_selected_half_index >= 1) && (m_selected_half_index <= HALFS))
                    {
                        folder_name += "/" + m_selected_half_index.ToString();     // sub filepath: a single half
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if (m_selected_half_index == HALFS + 1)  // few halfs
                    ...
                    else if (m_selected_half_index == HALFS + 2)  // half by half each in its own filepath
                    {
                    }
        ...
        private void BuildNetworkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (m_build_network_halfs_1by1)
                {
                    BuildNetworkForEachHalfs();
                }
                else
                {
                    BuildNetwork();
        ...
        private void BuildNetworkForEachHalfs()
        {
            for (int i = 1; i <= HALFS; i++)
            {
                m_selected_half_index = i;
                m_selected_half_numbers = new List<int>() { m_selected_half_index };
        ...
        private void SaveNetwork()
        {
            ...
            if (m_nodes != null)
            {
                ...
                if (m_nodes.Count > 0)
                {
                    // (m_selected_half_index == 0) // all halfs
                    foreach (Node node in m_nodes)
                    {
                        str.Append("\t\t{");
                        str.Append("\"id\":" + " " + node.Id.ToString() + ", ");
                        str.Append("\"fx\":" + " " + ((m_build_network_halfs_1by1 ? 0 : node.X) * 10 - 800).ToString() + ", ");
