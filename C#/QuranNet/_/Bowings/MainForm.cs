...
namespace QuranNet
{
    public partial class MainForm : Form
    {
        ...
        {
            BowingsComboBox.Items.Clear();
            BowingsComboBox.Items.Add("All");  // all bowings
            for (int i = 1; i <= BOWINGS; i++)
            {
                BowingsComboBox.Items.Add(i.ToString());
            }
            BowingsComboBox.Items.Add("Few");      // speical bowings
            BowingsComboBox.Items.Add("1by1");     // bowing by bowing each in its own filepath
            BowingsComboBox.SelectedIndex = BOWINGS + 1;
        ...
        private List<int> m_special_bowing_numbers = null;
        private void LoadSpecialBowingNumbers()
        {
            m_special_bowing_numbers = new List<int>();
            string special_bowings_filename = "3dQuran" + Path.DirectorySeparatorChar + "bowings.txt";
            if (File.Exists(special_bowings_filename))
            {
                List<string> lines = FileHelper.LoadLines(special_bowings_filename);
                foreach (string line in lines)
                {
                    int special_bowing_number;
                    if (int.TryParse(line, out special_bowing_number))
                    {
                        m_special_bowing_numbers.Add(special_bowing_number);
                    }
                }
            }
        }
        private List<int> m_selected_bowing_numbers = null;
        ...
        private bool m_build_network_bowings_1by1 = false;
        private int m_selected_bowing_index = -1;
        private void BowingsComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            m_selected_bowing_index = BowingsComboBox.SelectedIndex;
            if (m_selected_bowing_index > -1)
            {
                if (m_selected_bowing_index == BowingsComboBox.Items.Count - 1) // bowing by bowing each in its own filepath
                {
                    m_build_network_bowings_1by1 = true;
                    m_selected_bowing_numbers = new List<int>();
                    ToolTip.SetToolTip(this.BowingsComboBox, "Build network bowing by bowing each in its own filepath");
                }
                else if (m_selected_bowing_index == BOWINGS + 1) // few bowings
                {
                    m_build_network_bowings_1by1 = false;
                    m_selected_bowing_numbers = m_special_bowing_numbers;
                    ToolTip.SetToolTip(this.BowingsComboBox, "Build network for special bowings");
                }
                else if (m_selected_bowing_index == 0)                                 // all bowings
                {
                    m_build_network_bowings_1by1 = false;
                    m_selected_bowing_numbers = new List<int>();
                    for (int i = 1; i <= BOWINGS; i++)
                    {
                        m_selected_bowing_numbers.Add(i);
                    }
                    ToolTip.SetToolTip(this.BowingsComboBox, "Build network for all bowings");
                }
                else if ((m_selected_bowing_index > 0) && (m_selected_bowing_index <= BOWINGS))   // a single bowing
                {
                    m_build_network_bowings_1by1 = false;
                    m_selected_bowing_numbers = new List<int>() { m_selected_bowing_index };
                    ToolTip.SetToolTip(this.BowingsComboBox, "Build network for bowing " + m_selected_bowing_index.ToString());
                }
            }
        }
        ...
        private void BuildNodes()
        {
            try
            {
                if (m_selected_bowing_numbers != null)
                {
                    ...
                    if (m_nodes != null)
                    {
                        ...
                        if (m_client != null)
                        {
                            ...
                            {
                                foreach (Bowing bowing in m_client.Book.Bowings)
                                {
                                    if (m_selected_bowing_numbers.Contains(bowing.Number))
                                    {
                                        foreach (Verse verse in bowing.Verses)
                                        {
                                            foreach (Word word in verse.Words)
                                            {
                                                ...
                                                node.Id = word.Number;
                                                node.X = bowing.Number;
                                                node.Y = verse.NumberInBowing;
        ...
        private void SaveLinks()
        {
            if (m_links != null)
            {
                ...
                if (Directory.Exists(DATA_FOLDER))
                {
                    ...
                    if (m_selected_bowing_index == 0)
                    {
                        folder_name += "/" + "all";                             // sub filepath: all bowings
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if ((m_selected_bowing_index >= 1) && (m_selected_bowing_index <= BOWINGS))
                    {
                        folder_name += "/" + m_selected_bowing_index.ToString();     // sub filepath: a single bowing
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if (m_selected_bowing_index == BOWINGS + 1)  // few bowings
                    ...
                    else if (m_selected_bowing_index == BOWINGS + 2)  // bowing by bowing each in its own filepath
                    {
                    }
        ...
        private void BuildNetworkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (m_build_network_bowings_1by1)
                {
                    BuildNetworkForEachBowings();
                }
                else
                {
                    BuildNetwork();
        ...
        private void BuildNetworkForEachBowings()
        {
            for (int i = 1; i <= BOWINGS; i++)
            {
                m_selected_bowing_index = i;
                m_selected_bowing_numbers = new List<int>() { m_selected_bowing_index };
        ...
        private void SaveNetwork()
        {
            ...
            if (m_nodes != null)
            {
                ...
                if (m_nodes.Count > 0)
                {
                    // (m_selected_bowing_index == 0) // all bowings
                    foreach (Node node in m_nodes)
                    {
                        str.Append("\t\t{");
                        str.Append("\"id\":" + " " + node.Id.ToString() + ", ");
                        str.Append("\"fx\":" + " " + ((m_build_network_bowings_1by1 ? 0 : node.X) * 10 - 800).ToString() + ", ");
