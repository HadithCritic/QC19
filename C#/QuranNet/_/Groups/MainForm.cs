...
namespace QuranNet
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Shown(object sender, EventArgs e)
        {
            ...
            GroupsComboBox.Items.Add("All");  // all groups
            for (int i = 1; i <= GROUPS; i++)
            {
                GroupsComboBox.Items.Add(i.ToString());
            }
            GroupsComboBox.Items.Add("Few");      // speical groups
            GroupsComboBox.Items.Add("1by1");     // group by group each in its own filepath
            GroupsComboBox.SelectedIndex = GROUPS + 1;
        ...
        private List<int> m_special_group_numbers = null;
        private void LoadSpecialGroupNumbers()
        {
            m_special_group_numbers = new List<int>();
            string special_groups_filename = "3dQuran" + Path.DirectorySeparatorChar + "groups.txt";
            if (File.Exists(special_groups_filename))
            {
                List<string> lines = FileHelper.LoadLines(special_groups_filename);
                foreach (string line in lines)
                {
                    int special_group_number;
                    if (int.TryParse(line, out special_group_number))
                    {
                        m_special_group_numbers.Add(special_group_number);
                    }
                }
            }
        }
        private List<int> m_selected_group_numbers = null;
        ...
        private bool m_build_network_groups_1by1 = false;
        private int m_selected_group_index = -1;
        private void GroupsComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            m_selected_group_index = GroupsComboBox.SelectedIndex;
            if (m_selected_group_index > -1)
            {
                if (m_selected_group_index == GroupsComboBox.Items.Count - 1) // group by group each in its own filepath
                {
                    m_build_network_groups_1by1 = true;
                    m_selected_group_numbers = new List<int>();
                    ToolTip.SetToolTip(this.GroupsComboBox, "Build network group by group each in its own filepath");
                }
                else if (m_selected_group_index == GROUPS + 1) // few groups
                {
                    m_build_network_groups_1by1 = false;
                    m_selected_group_numbers = m_special_group_numbers;
                    ToolTip.SetToolTip(this.GroupsComboBox, "Build network for special groups");
                }
                else if (m_selected_group_index == 0)                                 // all groups
                {
                    m_build_network_groups_1by1 = false;
                    m_selected_group_numbers = new List<int>();
                    for (int i = 1; i <= GROUPS; i++)
                    {
                        m_selected_group_numbers.Add(i);
                    }
                    ToolTip.SetToolTip(this.GroupsComboBox, "Build network for all groups");
                }
                else if ((m_selected_group_index > 0) && (m_selected_group_index <= GROUPS))   // a single group
                {
                    m_build_network_groups_1by1 = false;
                    m_selected_group_numbers = new List<int>() { m_selected_group_index };
                    ToolTip.SetToolTip(this.GroupsComboBox, "Build network for group " + m_selected_group_index.ToString());
                }
            }
        }
        ...
        private void BuildNodes()
        {
            try
            {
                if (m_selected_group_numbers != null)
                {
                    ...
                    if (m_nodes != null)
                    {
                        ...
                        if (m_client != null)
                        {
                            ...
                            {
                                foreach (Group group in m_client.Book.Groups)
                                {
                                    if (m_selected_group_numbers.Contains(group.Number))
                                    {
        ...
        private void SaveLinks()
        {
            if (m_links != null)
            {
                ...
                if (Directory.Exists(DATA_FOLDER))
                {
                    ...
                    if (m_selected_group_index == 0)
                    {
                        folder_name += "/" + "all";                             // sub filepath: all groups
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if ((m_selected_group_index >= 1) && (m_selected_group_index <= GROUPS))
                    {
                        folder_name += "/" + m_selected_group_index.ToString();     // sub filepath: a single group
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if (m_selected_group_index == GROUPS + 1)  // few groups
                    ...
                    else if (m_selected_group_index == GROUPS + 2)  // group by group each in its own filepath
                    {
                    }
        ...
        private void BuildNetworkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (m_build_network_groups_1by1)
                {
                    BuildNetworkForEachGroups();
                }
                else
                {
                    BuildNetwork();
        ...
        private void BuildNetworkForEachGroups()
        {
            for (int i = 1; i <= GROUPS; i++)
            {
                m_selected_group_index = i;
                m_selected_group_numbers = new List<int>() { m_selected_group_index };
        ...
        private void SaveNetwork()
        {
            ...
            if (m_nodes != null)
            {
                ...
                if (m_nodes.Count > 0)
                {
                    // (m_selected_group_index == 0) // all groups
                    foreach (Node node in m_nodes)
                    {
                        str.Append("\t\t{");
                        str.Append("\"id\":" + " " + node.Id.ToString() + ", ");
                        str.Append("\"fx\":" + " " + ((m_build_network_groups_1by1 ? 0 : node.X) * 10 - 800).ToString() + ", ");
