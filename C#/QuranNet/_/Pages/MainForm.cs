...
namespace QuranNet
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Shown(object sender, EventArgs e)
        {
            ...
            PagesComboBox.Items.Add("All");  // all pages
            for (int i = 1; i <= PAGES; i++)
            {
                PagesComboBox.Items.Add(i.ToString());
            }
            PagesComboBox.Items.Add("Few");      // speical pages
            PagesComboBox.Items.Add("1by1");     // page by page each in its own filepath
            PagesComboBox.SelectedIndex = PAGES + 1;
        ...
        private List<int> m_special_page_numbers = null;
        private void LoadSpecialPageNumbers()
        {
            m_special_page_numbers = new List<int>();
            string special_pages_filename = "3dQuran" + Path.DirectorySeparatorChar + "pages.txt";
            if (File.Exists(special_pages_filename))
            {
                List<string> lines = FileHelper.LoadLines(special_pages_filename);
                foreach (string line in lines)
                {
                    int special_page_number;
                    if (int.TryParse(line, out special_page_number))
                    {
                        m_special_page_numbers.Add(special_page_number);
                    }
                }
            }
        }
        private List<int> m_selected_page_numbers = null;
        ...
        private bool m_build_network_pages_1by1 = false;
        private int m_selected_page_index = -1;
        private void PagesComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            m_selected_page_index = PagesComboBox.SelectedIndex;
            if (m_selected_page_index > -1)
            {
                if (m_selected_page_index == PagesComboBox.Items.Count - 1) // page by page each in its own filepath
                {
                    m_build_network_pages_1by1 = true;
                    m_selected_page_numbers = new List<int>();
                    ToolTip.SetToolTip(this.PagesComboBox, "Build network page by page each in its own filepath");
                }
                else if (m_selected_page_index == PAGES + 1) // few pages
                {
                    m_build_network_pages_1by1 = false;
                    m_selected_page_numbers = m_special_page_numbers;
                    ToolTip.SetToolTip(this.PagesComboBox, "Build network for special pages");
                }
                else if (m_selected_page_index == 0)                                 // all pages
                {
                    m_build_network_pages_1by1 = false;
                    m_selected_page_numbers = new List<int>();
                    for (int i = 1; i <= PAGES; i++)
                    {
                        m_selected_page_numbers.Add(i);
                    }
                    ToolTip.SetToolTip(this.PagesComboBox, "Build network for all pages");
                }
                else if ((m_selected_page_index > 0) && (m_selected_page_index <= PAGES))   // a single page
                {
                    m_build_network_pages_1by1 = false;
                    m_selected_page_numbers = new List<int>() { m_selected_page_index };
                    ToolTip.SetToolTip(this.PagesComboBox, "Build network for page " + m_selected_page_index.ToString());
                }
            }
        }
        ...
        private void BuildNodes()
        {
            try
            {
                if (m_selected_page_numbers != null)
                {
                    ...
                    if (m_nodes != null)
                    {
                        ...
                        if (m_client != null)
                        {
                            ...
                            {
                                foreach (Page page in m_client.Book.Pages)
                                {
                                    if (m_selected_page_numbers.Contains(page.Number))
                                    {
        ...
        private void SaveLinks()
        {
            if (m_links != null)
            {
                ...
                if (Directory.Exists(DATA_FOLDER))
                {
                    ...
                    {
                        folder_name += "/" + "all";                             // sub filepath: all pages
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if ((m_selected_page_index >= 1) && (m_selected_page_index <= PAGES))
                    {
                        folder_name += "/" + m_selected_page_index.ToString();     // sub filepath: a single page
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if (m_selected_page_index == PAGES + 1)  // few pages
                    ...
                    else if (m_selected_page_index == PAGES + 2)  // page by page each in its own filepath
                    {
                    }
        ...
        private void BuildNetworkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (m_build_network_pages_1by1)
                {
                    BuildNetworkForEachPages();
                }
                else
                {
                    BuildNetwork();
        ...
        private void BuildNetworkForEachPages()
        {
            for (int i = 1; i <= PAGES; i++)
            {
                m_selected_page_index = i;
                m_selected_page_numbers = new List<int>() { m_selected_page_index };
        ...
        private void SaveNetwork()
        {
            ...
            if (m_nodes != null)
            {
                ...
                if (m_nodes.Count > 0)
                {
                    // (m_selected_page_index == 0) // all pages
                    foreach (Node node in m_nodes)
                    {
                        str.Append("\t\t{");
                        str.Append("\"id\":" + " " + node.Id.ToString() + ", ");
                        str.Append("\"fx\":" + " " + ((m_build_network_pages_1by1 ? 0 : node.X) * 10 - 800).ToString() + ", ");
