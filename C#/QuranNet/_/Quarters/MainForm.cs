...
namespace QuranNet
{
    public partial class MainForm : Form
    {
        ...
        private const int QUARTERS = 240;
        ...
        {
            QuartersComboBox.Items.Clear();
            QuartersComboBox.Items.Add("All");  // all quarters
            for (int i = 1; i <= QUARTERS; i++)
            {
                QuartersComboBox.Items.Add(i.ToString());
            }
            QuartersComboBox.Items.Add("Few");      // speical quarters
            QuartersComboBox.Items.Add("1by1");     // quarter by quarter each in its own filepath
            QuartersComboBox.SelectedIndex = QUARTERS + 1;
        ...
        private List<int> m_special_quarter_numbers = null;
        private void LoadSpecialQuarterNumbers()
        {
            m_special_quarter_numbers = new List<int>();
            string special_quarters_filename = "3dQuran" + Path.DirectorySeparatorChar + "quarters.txt";
            if (File.Exists(special_quarters_filename))
            {
                List<string> lines = FileHelper.LoadLines(special_quarters_filename);
                foreach (string line in lines)
                {
                    int special_quarter_number;
                    if (int.TryParse(line, out special_quarter_number))
                    {
                        m_special_quarter_numbers.Add(special_quarter_number);
                    }
                }
            }
        }
        private List<int> m_selected_quarter_numbers = null;
        ...
        private bool m_build_network_quarters_1by1 = false;
        private int m_selected_quarter_index = -1;
        private void QuartersComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            m_selected_quarter_index = QuartersComboBox.SelectedIndex;
            if (m_selected_quarter_index > -1)
            {
                if (m_selected_quarter_index == QuartersComboBox.Items.Count - 1) // quarter by quarter each in its own filepath
                {
                    m_build_network_quarters_1by1 = true;
                    m_selected_quarter_numbers = new List<int>();
                    ToolTip.SetToolTip(this.QuartersComboBox, "Build network quarter by quarter each in its own filepath");
                }
                else if (m_selected_quarter_index == QUARTERS + 1) // few quarters
                {
                    m_build_network_quarters_1by1 = false;
                    m_selected_quarter_numbers = m_special_quarter_numbers;
                    ToolTip.SetToolTip(this.QuartersComboBox, "Build network for special quarters");
                }
                else if (m_selected_quarter_index == 0)                                 // all quarters
                {
                    m_build_network_quarters_1by1 = false;
                    m_selected_quarter_numbers = new List<int>();
                    for (int i = 1; i <= QUARTERS; i++)
                    {
                        m_selected_quarter_numbers.Add(i);
                    }
                    ToolTip.SetToolTip(this.QuartersComboBox, "Build network for all quarters");
                }
                else if ((m_selected_quarter_index > 0) && (m_selected_quarter_index <= QUARTERS))   // a single quarter
                {
                    m_build_network_quarters_1by1 = false;
                    m_selected_quarter_numbers = new List<int>() { m_selected_quarter_index };
                    ToolTip.SetToolTip(this.QuartersComboBox, "Build network for quarter " + m_selected_quarter_index.ToString());
                }
            }
        }
        ...
        private void BuildNodes()
        {
            try
            {
                if (m_selected_quarter_numbers != null)
                {
                    ...
                    if (m_nodes != null)
                    {
                        ...
                        if (m_client != null)
                        {
                            ...
                            {
                                foreach (Quarter quarter in m_client.Book.Quarters)
                                {
                                    if (m_selected_quarter_numbers.Contains(quarter.Number))
                                    {
                                        foreach (Verse verse in quarter.Verses)
                                        {
                                            foreach (Word word in verse.Words)
                                            {
                                                ...
                                                node.Id = word.Number;
                                                node.X = quarter.Number;
                                                node.Y = verse.NumberInQuarter;
        ...
        private void SaveLinks()
        {
            if (m_links != null)
            {
                ...
                if (Directory.Exists(DATA_FOLDER))
                {
                    ...
                    if (m_selected_quarter_index == 0)
                    {
                        folder_name += "/" + "all";                             // sub filepath: all quarters
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if ((m_selected_quarter_index >= 1) && (m_selected_quarter_index <= QUARTERS))
                    {
                        folder_name += "/" + m_selected_quarter_index.ToString();     // sub filepath: a single quarter
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if (m_selected_quarter_index == QUARTERS + 1)  // few quarters
                    ...
                    else if (m_selected_quarter_index == QUARTERS + 2)  // quarter by quarter each in its own filepath
                    {
                    }
        ...
        private void BuildNetworkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (m_build_network_quarters_1by1)
                {
                    BuildNetworkForEachQuarters();
                }
                else
                {
                    BuildNetwork();
        ...
        private void BuildNetworkForEachQuarters()
        {
            for (int i = 1; i <= QUARTERS; i++)
            {
                m_selected_quarter_index = i;
                m_selected_quarter_numbers = new List<int>() { m_selected_quarter_index };
        ...
        private void SaveNetwork()
        {
            ...
            if (m_nodes != null)
            {
                ...
                if (m_nodes.Count > 0)
                {
                    // (m_selected_quarter_index == 0) // all quarters
                    foreach (Node node in m_nodes)
                    {
                        str.Append("\t\t{");
                        str.Append("\"id\":" + " " + node.Id.ToString() + ", ");
                        str.Append("\"fx\":" + " " + ((m_build_network_quarters_1by1 ? 0 : node.X) * 10 - 800).ToString() + ", ");
                        str.Append("\"fy\":" + " " + (node.Y * 100 - 400).ToString() + ", ");
                        str.Append("\"fz\":" + " " + (node.Z * 10 + 1800).ToString() + ", ");
