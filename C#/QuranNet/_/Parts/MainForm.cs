...
namespace QuranNet
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Shown(object sender, EventArgs e)
        {
            ...
            PartsComboBox.Items.Add("All");  // all parts
            for (int i = 1; i <= PARTS; i++)
            {
                PartsComboBox.Items.Add(i.ToString());
            }
            PartsComboBox.Items.Add("Few");      // speical parts
            PartsComboBox.Items.Add("1by1");     // part by part each in its own filepath
            PartsComboBox.SelectedIndex = PARTS + 1;
        ...
        private List<int> m_special_part_numbers = null;
        private void LoadSpecialPartNumbers()
        {
            m_special_part_numbers = new List<int>();
            string special_parts_filename = "3dQuran" + Path.DirectorySeparatorChar + "parts.txt";
            if (File.Exists(special_parts_filename))
            {
                List<string> lines = FileHelper.LoadLines(special_parts_filename);
                foreach (string line in lines)
                {
                    int special_part_number;
                    if (int.TryParse(line, out special_part_number))
                    {
                        m_special_part_numbers.Add(special_part_number);
                    }
                }
            }
        }
        private List<int> m_selected_part_numbers = null;
        ...
        private bool m_build_network_parts_1by1 = false;
        private int m_selected_part_index = -1;
        private void PartsComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            m_selected_part_index = PartsComboBox.SelectedIndex;
            if (m_selected_part_index > -1)
            {
                if (m_selected_part_index == PartsComboBox.Items.Count - 1) // part by part each in its own filepath
                {
                    m_build_network_parts_1by1 = true;
                    m_selected_part_numbers = new List<int>();
                    ToolTip.SetToolTip(this.PartsComboBox, "Build network part by part each in its own filepath");
                }
                else if (m_selected_part_index == PARTS + 1) // few parts
                {
                    m_build_network_parts_1by1 = false;
                    m_selected_part_numbers = m_special_part_numbers;
                    ToolTip.SetToolTip(this.PartsComboBox, "Build network for special parts");
                }
                else if (m_selected_part_index == 0)                                 // all parts
                {
                    m_build_network_parts_1by1 = false;
                    m_selected_part_numbers = new List<int>();
                    for (int i = 1; i <= PARTS; i++)
                    {
                        m_selected_part_numbers.Add(i);
                    }
                    ToolTip.SetToolTip(this.PartsComboBox, "Build network for all parts");
                }
                else if ((m_selected_part_index > 0) && (m_selected_part_index <= PARTS))   // a single part
                {
                    m_build_network_parts_1by1 = false;
                    m_selected_part_numbers = new List<int>() { m_selected_part_index };
                    ToolTip.SetToolTip(this.PartsComboBox, "Build network for part " + m_selected_part_index.ToString());
                }
            }
        }
        ...
        private void BuildNodes()
        {
            try
            {
                if (m_selected_part_numbers != null)
                {
                    ...
                    if (m_nodes != null)
                    {
                        ...
                        if (m_client != null)
                        {
                            ...
                            {
                                foreach (Part part in m_client.Book.Parts)
                                {
                                    if (m_selected_part_numbers.Contains(part.Number))
                                    {
        ...
        private void SaveLinks()
        {
            if (m_links != null)
            {
                ...
                if (Directory.Exists(DATA_FOLDER))
                {
                    ...
                    if (m_selected_part_index == 0)
                    {
                        folder_name += "/" + "all";                             // sub filepath: all parts
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if ((m_selected_part_index >= 1) && (m_selected_part_index <= PARTS))
                    {
                        folder_name += "/" + m_selected_part_index.ToString();     // sub filepath: a single part
                        Directory.CreateDirectory(folder_name);
                        SaveIndexFile(folder_name);
                    }
                    else if (m_selected_part_index == PARTS + 1)  // few parts
                    ...
                    else if (m_selected_part_index == PARTS + 2)  // part by part each in its own filepath
                    {
                    }
        ...
        private void BuildNetworkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (m_build_network_parts_1by1)
                {
                    BuildNetworkForEachParts();
                }
                else
                {
                    BuildNetwork();
        ...
        private void BuildNetworkForEachParts()
        {
            for (int i = 1; i <= PARTS; i++)
            {
                m_selected_part_index = i;
                m_selected_part_numbers = new List<int>() { m_selected_part_index };
        ...
        private void SaveNetwork()
        {
            ...
            if (m_nodes != null)
            {
                ...
                if (m_nodes.Count > 0)
                {
                    // (m_selected_part_index == 0) // all parts
                    foreach (Node node in m_nodes)
                    {
                        str.Append("\t\t{");
                        str.Append("\"id\":" + " " + node.Id.ToString() + ", ");
                        str.Append("\"fx\":" + " " + ((m_build_network_parts_1by1 ? 0 : node.X) * 10 - 800).ToString() + ", ");
